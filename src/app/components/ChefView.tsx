...
const INITIAL_ORDERS: Order[] = [
  ...
  },
  {
    ...
    status: 'Listo',
    items: [
      { id: 'i7', name: 'Ensalada César', quantity: 1 }
    ]
  }
];
...
export function ChefView() {
  ...
  const OrderCard = ({ order }: { order: Order }) => (
    <div className={`bg-white rounded-2xl shadow-md border-l-[6px] p-4 flex flex-col gap-4 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${
      order.status === 'Pendiente' ? 'border-l-yellow-400' : 
      order.status === 'En preparación' ? 'border-l-[#D0543A]' : 
      ...
      <div className="pt-3 mt-auto">
        {order.status === 'Pendiente' && (
          <button onClick={() => updateOrderStatus(order.id, 'En preparación')} className="w-full flex items-center justify-center gap-2 bg-yellow-400 hover:bg-yellow-500 text-yellow-900 font-black py-2.5 rounded-xl transition-colors shadow-sm">
            <Play size={16} /> Preparar
          </button>
        )}
        {order.status === 'En preparación' && (
          <button onClick={() => updateOrderStatus(order.id, 'Listo')} className="w-full flex items-center justify-center gap-2 bg-[#D0543A] hover:bg-[#b5462f] text-white font-black py-2.5 rounded-xl transition-colors shadow-md shadow-[#D0543A]/30">
  ...
  return (
    <div className="min-h-full bg-[#FCE4D6]/30 flex flex-col">
      ...
      <header className="px-6 sm:px-10 py-6 sticky top-0 bg-[#FCE4D6]/90 backdrop-blur-md z-10 border-b border-[#E57C5D]/20 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[#4B2E2D]/70 font-medium text-sm">Panel de control de comandas en tiempo real</p>
          </div>
        </div>
      ...
      <div className="flex-1 p-6 sm:p-10 grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 items-start">
        ...
        <div className="flex flex-col gap-4 bg-white/50 p-4 rounded-3xl border-2 border-yellow-200/50 min-h-[500px]"><div className="flex items-center justify-between mb-2"><h2 className="text-xl font-black text-[#4B2E2D] flex items-center gap-2"><Clock className="text-yellow-500" /> Por hacer</h2><span className="bg-yellow-200 text-yellow-800 font-black px-3 py-1 rounded-full text-sm">{pendingOrders.length}</span></div>{pendingOrders.map(order => <OrderCard key={order.id} order={order} />)}{pendingOrders.length === 0 && <p className="text-center text-[#4B2E2D]/40 font-medium py-10">No hay pedidos pendientes</p>}</div>

        {/* Columna 2: En Preparación */}
        <div className="flex flex-col gap-4 bg-[#FCE4D6]/40 p-4 rounded-3xl border-2 border-[#D0543A]/20 min-h-[500px]"><div className="flex items-center justify-between mb-2"><h2 className="text-xl font-black text-[#4B2E2D] flex items-center gap-2"><Flame className="text-[#D0543A]" /> Cocinando</h2><span className="bg-[#D0543A] text-white font-black px-3 py-1 rounded-full text-sm">{prepOrders.length}</span></div>{prepOrders.map(order => <OrderCard key={order.id} order={order} />)}{prepOrders.length === 0 && <p className="text-center text-[#4B2E2D]/40 font-medium py-10">No hay platos en preparación</p>}</div>

        {/* Columna 3: Listos */}
        <div className="flex flex-col gap-4 bg-emerald-50/50 p-4 rounded-3xl border-2 border-emerald-200/50 min-h-[500px]"><div className="flex items-center justify-between mb-2"><h2 className="text-xl font-black text-[#4B2E2D] flex items-center gap-2"><CheckCircle2 className="text-emerald-500" /> Listos</h2><span className="bg-emerald-200 text-emerald-800 font-black px-3 py-1 rounded-full text-sm">{readyOrders.length}</span></div>{readyOrders.map(order => <OrderCard key={order.id} order={order} />)}{readyOrders.length === 0 && <p className="text-center text-[#4B2E2D]/40 font-medium py-10">No hay pedidos para recoger</p>}</div>
      </div>
    </div>
  );
}