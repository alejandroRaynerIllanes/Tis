...
export function TableSidePanel({ isOpen, tableId, onClose, onOpenPayment }: TableSidePanelProps) {
  const {
    products,
    tables,
    orders,
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      api
        .get(`/pedidos?mesa=${tableId}&activo=true`)
        .then((res: any) => {
            ...
            <div className="p-4 space-y-4 pb-8">
              <div className="relative group mb-4">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-white border border-[#FCE4D6] rounded-xl text-base sm:text-sm font-semibold text-[#4B2E2D] placeholder:text-[#4B2E2D]/40 focus:outline-none focus:border-[#E57C5D] transition-all shadow-sm"
                />
              </div>
              <div className="space-y-3">
                {filteredDishes.map((dish) => {
                  ...
                  return (
                    <div
                      key={dish.id}
                      className={`flex items-center gap-4 bg-white p-3 rounded-[16px] transition-all duration-300 border border-gray-100 ${!isActive ? 'opacity-60 bg-gray-50' : 'hover:border-[#D0543A]/30 hover:shadow-md'}`}
                    >
              ...
              {activeOrder.length === 0 ? (
                ...
                <div className="flex flex-col h-full">
                  <div className="flex justify-between items-center mb-4 text-[#4B2E2D]">
                    ...
                    {backendOrder ? (
                      <span
                        className="text-[10px] font-mono bg-gray-100 text-[#D0543A] font-bold px-2 py-1 rounded border border-gray-200 shadow-sm"
                        title="ID del Pedido"
                      >
                        {backendOrder.codigo ||
                          `PED-${String(backendOrder._id || backendOrder.id)
                            .slice(-4)
                            .toUpperCase()}`}
                      </span>
                    ) : (
                      <span
                        className="text-[10px] font-mono bg-orange-50 text-orange-500 px-2 py-1 rounded border border-orange-200 shadow-sm"
                        title="Se generará al enviar"
                      >
                        ID: Pendiente
                      </span>
                    )}
                ...
                <div className="space-y-3 pb-24 overflow-y-auto">
                  {backendOrder ? (
                    <div className="bg-gray-50 border border-gray-100 p-2.5 rounded-xl flex justify-between items-center text-xs shadow-sm mb-1">
                      ...
                      <span className="font-mono font-black text-[#D0543A]">
                        {backendOrder.codigo ||
                          `PED-${String(backendOrder._id || backendOrder.id)
                            .slice(-4)
                            .toUpperCase()}`}
                        ...
                        {isEditingNote ? (
                          <div className="flex gap-2 mt-1">
                            <input
                              ...
                              className="flex-1 text-base sm:text-sm bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-[#D0543A]"
                              autoFocus
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                  updateOrderItemNote(
                          ...
                          <button
                            onClick={() =>
                              setEditingNote({ productId: item.product.id, text: item.note || '' })
                            }
                          ...
                          >
                            {item.note ? (
                              <>
                                ...
                                <span className="italic max-w-[200px] truncate">{item.note}</span>{' '}
          ...
          <div className="p-5 sm:p-6 bg-white border-t border-[#FCE4D6]/60 shadow-[0_-8px_20px_-10px_rgba(0,0,0,0.1)] shrink-0 flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-3">
              {viewingMenu ? (
                ...
                >
                  <ShoppingBag size={20} /> Ver Pedido Actual
                </button>
              ) : showSummary ? (
                <button
                ...
                >
                  {isSubmitting ? (
                    ...
                    <>
                      <CheckCircle2 size={20} /> Confirmar y Enviar a Cocina
                    </>
                  )}
                </button>
                ...
                >
                  <CheckCircle2 size={20} /> Ver Resumen de Orden
                </button>
              ) : (
                <button
                  onClick={() => setViewingMenu(true)}
                  className="w-full py-4 rounded-2xl bg-[#F5E6D3] border-2 border-[#6B3E2E] text-[#2C2C2C] font-black shadow-sm transition-all text-[15px] flex items-center justify-center gap-2 hover:bg-[#E8D4BE]"
                >
                  <Plus size={20} /> Añadir Más Platos
                </button>
              )}
                ...
                <>
                  ...
                  {canRequestBill && (
                    <button
                      ...
                      className="w-full py-4 rounded-2xl bg-[#E6A23C] hover:bg-[#d59330] text-white font-black shadow-lg shadow-[#E6A23C]/20 transition-all text-[15px] flex items-center justify-center gap-2 mt-1 mb-1"
                    >
                  ...
                  >
                    <Trash2 size={18} /> Cancelar Pedido Completo
                  </button>
                </>
              )}
            </div>
