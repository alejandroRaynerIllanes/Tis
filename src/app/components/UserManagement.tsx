...
import { 
  Users, UserPlus, Search, Edit2, Shield, ChefHat, 
  UtensilsCrossed, CreditCard, Ban, CheckCircle2,
  X, Save, Trash2, Loader2, RefreshCw
} from 'lucide-react';
...
export function UserManagement() {
  ...
  const getRoleIcon = (role: Role) => {
    switch(role) {
      case 'Administrador': return <Shield size={18} />;
      case 'Cocinero': return <ChefHat size={18} />;
      case 'Mesero': return <UtensilsCrossed size={18} />;
      case 'Cajero': return <CreditCard size={18} />;
    }
  };

  const getRoleColor = (role: Role) => {
    switch(role) {
      case 'Administrador': return 'bg-[#6B3E2E] text-white border-[#4B2E2D]';
      case 'Cocinero': return 'bg-[#D96C4A] text-white border-[#C55A38]';
      case 'Mesero': return 'bg-[#E6A23C] text-[#2C2C2C] border-[#D4912B]';
      case 'Cajero': return 'bg-[#F5E6D3] text-[#2C2C2C] border-[#E3D3BF]';
    }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 max-w-[1600px] mx-auto">
            {filteredUsers.map(user => (
              ...
              >
                ...
                <div className="relative z-10 mb-4 flex-1">
                  <h3 className="text-xl font-bold text-[#2C2C2C] leading-tight mb-1">{user.displayName}</h3>
                  ...
                  <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-bold border ${getRoleColor(user.displayRole)} shadow-sm`}>
                ...
                <div className="relative z-10 grid grid-cols-3 gap-2 mt-4 pt-4 border-t border-gray-100">
                  ...
                  >
                    {actionLoading === user.id ? <Loader2 size={14} className="animate-spin" /> : user.estado ? <Ban size={14} /> : <CheckCircle2 size={14} />}
            ...
            {filteredUsers.length === 0 && !loading && (
              <div className="col-span-full flex flex-col items-center justify-center py-20 bg-white/40 backdrop-blur-md rounded-[32px] border border-white/30">
                ...
                <h3 className="text-2xl font-bold text-white drop-shadow-sm mb-2">No se encontraron usuarios</h3>
          ...
          <div className="relative z-10 w-full max-w-md bg-white rounded-[28px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="bg-gradient-to-r from-[#6B3E2E] to-[#4B2E2D] px-6 py-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center backdrop-blur-md">
                  {editingUser ? <Edit2 size={20} className="text-white" /> : <UserPlus size={20} className="text-white" />}
                </div>
                <div>
                  ...
                  <p className="text-[#F5E6D3] text-xs font-semibold mt-0.5">
                    {editingUser ? 'Modifica los datos del personal' : 'Completa el formulario para registrar'}
                  </p>
                </div>
              </div>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-5 max-h-[70vh] overflow-y-auto">
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-[#4B2E2D] mb-1.5 ml-1">Nombre(s)</label>
                  <input
                    required
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-bold text-[#4B2E2D] mb-1.5 ml-1">Apellido(s)</label>
                  <input
                    required
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-bold text-[#4B2E2D] mb-1.5 ml-1">Correo Electrónico</label>
                  <input
                    required
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-bold text-[#4B2E2D] mb-1.5 ml-1">
                    Contraseña {editingUser && <span className="text-gray-400 font-normal text-xs">(Dejar en blanco para mantener actual)</span>}
                  </label>
                  <input
                ...
                <div>
                  <label className="block text-sm font-bold text-[#4B2E2D] mb-2 ml-1">Rol en el sistema</label>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-sm bg-white rounded-[24px] shadow-2xl p-6 text-center">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Se eliminará permanentemente a <strong>{deleteConfirm.displayName}</strong>. Esta acción no se puede deshacer.
            </p>
            <div className="flex gap-3">
              ...
              >
                {actionLoading === deleteConfirm.id ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}
                Eliminar
              </button>
            </div>
          </div>
