...
export const NotificationsProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addNotification = useCallback((notification: Omit<AppNotification, 'id' | 'time' | 'read'>): boolean => {
    const prev = notificationsRef.current

    // Prevent duplicates for 'Cuenta Solicitada'
    if (notification.title === 'Cuenta Solicitada' && notification.meta?.tableId) {
      if (prev.some(n => !n.read && n.meta?.tableId === notification.meta?.tableId && n.title === 'Cuenta Solicitada')) {
        return false
      }
    }
    ...
    if (notification.title === 'Pedido Listo' && notification.meta?.pedidoId) {
      if (prev.some(n => n.meta?.pedidoId === notification.meta?.pedidoId && n.title === 'Pedido Listo')) {
        return false
      }
    }
