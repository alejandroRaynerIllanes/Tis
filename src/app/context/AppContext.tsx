...
export const AppProvider = ({ children }: { children: ReactNode }) => {
      ...
      if (updatedOrder.length === 0) {
        setTimeout(() => {
          setReservations(currentReservations => {
            const hasReservations = currentReservations[tableId] && currentReservations[tableId].length > 0;
            
            if (!hasReservations) {
              setTables(current => current.map(t => 
                t.id === tableId ? { ...t, status: 'Disponible' } : t
              ));
            }
  ...
  const reserveTable = (tableId: string, info: Omit<ReservationInfo, 'id' | 'endTime' | 'vip'>) => {
    ...
    setTables(current => current.map(t => t.id === tableId ? { ...t, status: 'Reservada' } : t));
  };
  ...
  const cancelReservation = (tableId: string, reservationId: string) => {
    ...
    const remainingReservations = (reservations[tableId] || []).filter(r => r.id !== reservationId);
    ...
    if (remainingReservations.length === 0) {
      ...
      setTables(current => current.map(t => t.id === tableId ? { ...t, status: 'Disponible' } : t));
    }
  };
