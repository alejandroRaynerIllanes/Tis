...
// Tipo del formulario de reserva — mapea a ReservationInfo pero usa nombres más simples durante la entrada
...
function getStateIcon(state: TableStatus, size = 22) {
  switch (state) {
    case 'Disponible':     return <CheckCircle2 size={size} strokeWidth={2.2} />;
    case 'Ocupada':        return <Users         size={size} strokeWidth={2.2} />;
    case 'Esperando pago': return <Receipt       size={size} strokeWidth={2.2} />;
    case 'Reservada':      return <CalendarDays  size={size} strokeWidth={2.2} />;
    default: return null;
  }
}
...
export function WaiterView({
  ...
  const [reservationErrors, setReservationErrors] = useState<Partial<Record<keyof ReservationFormData, string>>>({});
  ...
  const filteredTables = tables.filter(t => {
    const locationMatch = activeLocation === 'Todas' || 
      (activeLocation === 'Zona VIP' ? t.location === 'zona-vip' : t.location.toLowerCase() === activeLocation.toLowerCase());
  ...
  const locationTables = activeLocation === 'Todas' ? tables : tables.filter(t => t.location.toLowerCase() === activeLocation.toLowerCase());
  ...
  const vipClientNameGlobal = hasVipReservation ? activeTableResArr.find(r => r.vip)?.clientName : undefined;
  ...
  const handleConfirmOrder = () => {
    if (activeTableId) {
      ...
      setShowSummary(false);
      toast.success(isVipOrder ? '⚡ Pedido VIP enviado con prioridad' : '¡Pedido enviado a cocina!', {
  ...
  const generateReservationPDF = (tableName: string, clientName: string, guestCount: number, date: string, time: string) => {
    const doc = new jsPDF();
    ...
    doc.text(`Fecha: ${new Date(date).toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}`, leftMargin, yPosition);
    yPosition += lineHeight;
    ...
    doc.setFont('Helvetica', 'bold');
    doc.setFontSize(12);
    doc.text(`Confirmación: #${confirmationNumber}`, pageWidth / 2, pageHeight - 20, { align: 'center' });
  ...
  const handleProcessPayment = () => {
    if (!selectedPaymentMethod || !activeTableId) {
    ...
    setTimeout(() => {
      ...
      toast.success('Pago procesado correctamente', {
        description: `La mesa ha sido liberada. Método: ${selectedPaymentMethod}`,
        duration: 4000,
      });
    }, 1500);
  };
  ...
  return (
    ...
    >
      ...
      {isEmbedded && isAdmin ? (
        <header className="px-6 lg:px-10 py-6 lg:py-8 flex flex-row justify-between items-center gap-6 sticky top-0 bg-black/40 backdrop-blur-md z-20 shrink-0 transition-all">
          <div className="flex flex-col">
            <h1 className="text-3xl sm:text-4xl font-bold text-white mb-1.5 drop-shadow-sm">Gestión de Mesas</h1>
            <p className="text-white/80 font-medium drop-shadow-sm text-sm sm:text-base">Administra los espacios físicos de tu local y su capacidad</p>
          </div>
        ...
        <section className="relative z-10 flex-1 min-w-0 flex flex-col overflow-y-auto p-4 sm:p-5 lg:p-7 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] transition-all duration-300">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5 pb-20">
            {filteredTables.map(t => {
              ...
              const tableReservation = activeRes || (tableReservations.length > 0 ? tableReservations[0] : null);
                ...
                >
                  ...
                  <div className="flex justify-between items-start w-full mb-6 relative z-10">
                    <div className="flex items-center gap-2">
                      <span className={`font-black text-xl sm:text-2xl tracking-tight drop-shadow-sm leading-none mt-1 ${cfg.textClass}`}>
                        {t.name}
                      </span>
                      {t.type === 'vip' && (
                        <Crown size={16} className="text-yellow-300 drop-shadow-sm shrink-0 mt-1" strokeWidth={2.5} />
                      )}
                    </div>
                  ...
                  {t.status === 'Reservada' && tableReservation && (
                    <div 
                      ...
                      title={tableReservations.length > 1 ? `Ver ${tableReservations.length} reservas` : "Ver reserva"}
                    >
                      <div className="flex items-center gap-1.5 text-white justify-between">
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          <span className="text-[11px] font-black truncate">{tableReservation.clientName}</span>
                          {tableReservation.vip && (
                            <Crown size={10} className="shrink-0 text-yellow-300" strokeWidth={2.5} />
                          )}
                        </div>
                  ...
                  <div className="flex justify-between items-end w-full relative z-10">
                    ...
                    <div className={`flex items-center gap-1 text-[11px] font-bold px-2 py-1 rounded bg-black/5 ${cfg.textClass}`}>
              ...
              {isVipOrder && (
                <div className="flex items-center gap-2.5 bg-amber-400/15 border border-amber-400/30 rounded-xl px-3.5 py-2.5 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-yellow-200 font-black text-[11px] uppercase tracking-wider">Pedido con Prioridad VIP</p>
                    <p className="text-white/55 text-[10px] font-medium leading-snug mt-0.5">
                      {vipClientName ? `Cliente: ${vipClientName} · ` : ''}Este pedido encabeza la cola de cocina
                    </p>
                  </div>
                ...
                <div className={`flex p-1 rounded-xl shadow-inner border ${isVipOrder ? 'bg-white/10 border-white/15' : 'bg-white/60 border-[#FCE4D6]'}`}>
                  <button
                    ...
                    className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${
                      !viewingMenu 
                        ? (isVipOrder ? 'bg-white/20 text-white shadow-sm' : 'bg-white text-[#4B2E2D] shadow-sm')
                        : (isVipOrder ? 'text-white/50 hover:bg-white/10' : 'text-[#4B2E2D]/60 hover:bg-white/40')
                    }`}
                  >
                    Pedido Actual
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${
                      viewingMenu 
                        ? (isVipOrder ? 'bg-amber-500/80 text-white shadow-sm' : 'bg-[#4B2E2D] text-white shadow-sm')
                        : (isVipOrder ? 'text-white/50 hover:bg-white/10' : 'text-[#4B2E2D]/60 hover:bg-white/40')
                    }`}
                  >
                    Menú
                      ...
                      return (
                        ...
                        >
                          <div className="w-16 h-16 rounded-xl overflow-hidden shrink-0 relative bg-gray-100">
                            ...
                            {!isActive && (
                              <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
                                <span className="bg-red-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded shadow-sm">AGOTADO</span>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="flex-1 min-w-0 flex flex-col justify-center text-left py-0.5">
                            ...
                            <span className={`text-sm font-black mt-1 ${isActive ? 'text-[#D0543A]' : 'text-gray-400'}`}>
                  ...
                  {activeOrder.length === 0 ? (
                    ...
                    <div className="flex flex-col h-full">
                      <div className="flex items-center gap-2 mb-4 text-[#4B2E2D]">
                        <button onClick={() => setShowSummary(false)} className="p-1 hover:bg-black/5 rounded-full transition-colors">
                          <ChevronLeft size={20} />
                        </button>
                          ...
                          <div key={item.product.id} className="flex flex-col gap-2 bg-white p-4 rounded-[16px] border border-gray-100 shadow-sm">
                            <div className="flex items-center gap-4">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  <span className="text-xs font-bold text-gray-400">Bs. {item.product.price.toFixed(2)} c/u</span>
                                </div>
                              </div>
                            ...
                            {isEditingNote ? (
                              <div className="flex gap-2 mt-1">
                                <input
                                  ...
                                  onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                      updateOrderItemNote(activeTable.id, item.product.id, editingNote.text);
                                      setEditingNote(null);
                                    }
                                  }}
                                />
                              ...
                              <button
                                onClick={() => setEditingNote({ productId: item.product.id, text: item.note || '' })}
                                className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-[#D0543A] self-start mt-1"
                              >
                                {item.note ? (
                                  <>
                                    <MessageSquare size={12} /> <span className="italic max-w-[200px] truncate">{item.note}</span> <Edit2 size={10} className="ml-1" />
                                  </>
                                ) : (
                                  <>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[#F5E6D3] text-xs font-semibold mt-0.5">
                      {reservingTable?.name} · Capacidad {reservingTable?.capacity} pax
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  {reservationErrors.clientName && (
                    <p className="text-red-500 text-[11px] font-semibold mt-1">{reservationErrors.clientName}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="number"
                      min={1}
                      max={reservingTable?.capacity ?? 20}
                      placeholder="Ej: 4"
                  ...
                  {reservationErrors.guestCount && (
                    <p className="text-red-500 text-[11px] font-semibold mt-1">{reservationErrors.guestCount}</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2.5 rounded-xl border text-sm font-semibold text-[#4B2E2D] focus:outline-none transition-all ${reservationErrors.date ? 'border-red-400 bg-red-50 focus:border-red-500' : 'border-gray-200 bg-gray-50 focus:border-[#D96C4A] focus:bg-white'}`}
                    />
                    {reservationErrors.date && (
                      <p className="text-red-500 text-[11px] font-semibold mt-1">{reservationErrors.date}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-xs font-black text-[#4B2E2D] uppercase tracking-wider mb-1.5">
                    ...
                    <input
                      ...
                      className={`w-full px-3 py-2.5 rounded-xl border text-sm font-semibold text-[#4B2E2D] focus:outline-none transition-all ${reservationErrors.time ? 'border-red-400 bg-red-50 focus:border-red-500' : 'border-gray-200 bg-gray-50 focus:border-[#D96C4A] focus:bg-white'}`}
                    />
                    {reservationErrors.time && (
                      <p className="text-red-500 text-[11px] font-semibold mt-1">{reservationErrors.time}</p>
                    )}
                  </div>
                </div>
              </div>
      ...
      ══════════════════════════════════════════════════════════════ */}
      {cancelingReservationTableId && cancelingReservationId && (() => {
        ...
        const cancelingReservationInfo = tableReservations.find(r => r.id === cancelingReservationId) || null;
        return (
            ...
            <div className="relative z-10 w-full max-w-[400px] bg-white rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
              ...
              <div className="bg-gradient-to-r from-[#8C3A3A] to-[#7A2A2A] px-6 py-5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[#F5E6D3] text-xs font-semibold mt-0.5">
                      {cancelingTable?.name}{cancelingReservationInfo ? ` · ${cancelingReservationInfo.clientName}` : ''}
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="p-6">
                <div className="flex flex-col items-center text-center gap-4">
                  ...
                  <div>
                    <p className="font-black text-[#4B2E2D] text-base">¿Estás seguro de que deseas cancelar esta reserva?</p>
                    {cancelingReservationInfo && (
                      <div className="mt-3 p-3 bg-[#FFF5F0] rounded-xl border border-[#FCE4D6] text-left space-y-1.5">
                        <div className="flex items-center gap-2 text-[#4B2E2D]">
                          ...
                          <span className="text-sm font-black">{cancelingReservationInfo.clientName}</span>
                        ...
                        <div className="flex items-center gap-3">
                          <span className="flex items-center gap-1.5 text-xs font-bold text-[#4B2E2D]/80">
                            <Clock size={11} /> {cancelingReservationInfo.date} · {cancelingReservationInfo.startTime}
                          </span>
                          <span className="flex items-center gap-1.5 text-xs font-bold text-[#4B2E2D]/80">
      ...
      {/* ══════════════════════════════════════════════════════════════
          MODAL DE PAGO
      ══════════════════════════════════════════════════════════════ */}
      {showPaymentModal && activeTableId && activeTable && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          {/* Overlay */}
          <div 
          ...
          {/* Modal Content */}
          <div className="relative z-10 w-full max-w-[500px] bg-white shadow-2xl rounded-3xl flex flex-col transform transition-all duration-300 ease-out overflow-hidden max-h-[90vh] animate-in zoom-in-95">
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-white/90 mt-0.5">{activeTable.name}</p>
                  </div>
                </div>
                <button
                  onClick={closePaymentModal}
            ...
            <div className="flex-1 overflow-y-auto bg-gray-50 p-6">
                ...
                <div className="border-t border-dashed border-gray-300 pt-3 space-y-2">
                  ...
                  {discountPercent > 0 && (
                    <div className="flex justify-between items-center text-sm text-green-600">
                      ...
                      <span className="font-bold">- Bs. {(orderTotal * discountPercent / 100).toFixed(2)}</span>
                    </div>
                  )}
                  ...
                  {tipPercent > 0 && (
                    <div className="flex justify-between items-center text-sm text-[#D96C4A]">
                      ...
                      <span className="font-bold">+ Bs. {(orderTotal * tipPercent / 100).toFixed(2)}</span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                    ...
                    <span className="font-black text-2xl text-[#D96C4A]">
                      Bs. {(orderTotal * (1 - discountPercent/100) * (1 + tipPercent/100)).toFixed(2)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                  <label className="flex items-center gap-2 text-sm font-bold text-[#4B2E2D] mb-3">
                    <Wallet size={16} className="text-[#D96C4A]" />
                    Propina
                  </label>
                  <div className="flex gap-2">
                    {[0, 5, 10, 15].map(percent => (
                      <button
                        ...
                        className={`flex-1 py-2 px-2 rounded-lg text-xs font-bold transition-all ${
                          tipPercent === percent
                            ? 'bg-[#D96C4A] text-white shadow-md'
                            : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                        } disabled:opacity-50`}
                      >
                        {percent}%
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center justify-center gap-2 p-4 rounded-xl font-bold transition-all ${
                        selectedPaymentMethod === method.id
                          ? 'bg-[#D96C4A] text-white shadow-lg ring-2 ring-[#D96C4A] ring-offset-2'
                          : 'bg-gray-50 text-[#4B2E2D] hover:bg-gray-100 border border-gray-200'
                      } disabled:opacity-50`}
                    >
                      {method.icon}
