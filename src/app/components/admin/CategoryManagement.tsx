    ...
    try {
      if (categoryEditingId) {
        ...
        if (oldCat && oldCat.label !== updated.nombre) {
          setDishes(dishes.map(d => d.category === categoryEditingId ? { ...d, category: updated.nombre } : d));
        }
    ...
    <>
      <header className="px-10 py-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 sticky top-0 bg-[#FCE4D6]/90 backdrop-blur-md z-10">
        <div>
          ...
          <p className="text-[#4B2E2D]/70 font-medium">Gestiona las diferentes secciones de tu carta (BD Conectada)</p>
        </div>
        <button onClick={handleOpenAddCategoryModal} className="flex items-center gap-2 bg-[#D0543A] text-white px-6 py-3.5 rounded-xl font-bold shadow-lg shadow-[#D0543A]/30 hover:bg-[#b5462f] hover:shadow-xl hover:-translate-y-0.5 transition-all active:scale-[0.98]">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white w-full max-w-[400px] border-[6px] border-[#4B2E2D] rounded-3xl p-8 relative my-8 shadow-2xl">
            ...
            <form onSubmit={handleSaveCategory}>
              <input type="text" required value={categoryFormData.label} onChange={(e) => setCategoryFormData({ label: e.target.value.replace(/[^a-zA-ZáéíóúÁÉÍÓÚüÜñÑ\s]/g, '') })} className="w-full px-4 py-3 rounded-xl border-2 border-[#E57C5D] text-[#4B2E2D] focus:outline-none mb-6" placeholder="Ej: Platos Especiales" />
              <div className="flex justify-end gap-4">
                <button type="button" onClick={() => setIsCategoryModalOpen(false)} className="px-6 py-3 font-bold text-[#4B2E2D]/60">Cancelar</button>
                <button type="submit" disabled={isLoading} className={`px-8 py-3 text-white font-bold rounded-xl shadow-lg ${isLoading ? 'bg-gray-400' : 'bg-[#D0543A]'}`}>Guardar</button>
              </div>
            </form>
          </div>
        </div>
