...
async function run() {
  ...
  try {
    ...
    // Submit
    await Promise.all([
      page.waitForResponse(resp => resp.url().includes('/mesas') && resp.request().method() === 'POST' && resp.status() < 500, { timeout: 8000 }),
    ...
    const createdList = await page.evaluate(async (api, tkn, name) => {
      const r = await fetch(`${api}/mesas`, { headers: { Authorization: `Bearer ${tkn}` } });
      const data = await r.json();
      return data.filter(m => m.name === name);
    }, BACKEND_API, token, mesaName);

    if (!createdList || createdList.length === 0) throw new Error('Created mesa not found in backend');
    ...
    await Promise.all([
      page.waitForResponse(resp => resp.url().includes(`/mesas/${mesa.id || mesa._id}`) && resp.request().method() === 'PUT' && resp.status() < 500, { timeout: 8000 }),
    ...
    const updated = await page.evaluate(async (api, tkn, id) => {
      const r = await fetch(`${api}/mesas/${id}`, { headers: { Authorization: `Bearer ${tkn}` } });
      return r.json();
    }, BACKEND_API, token, mesa.id || mesa._id);

    if (!updated || Number(updated.capacity) !== 6) throw new Error('Updated capacity not reflected in backend');
    ...
    const afterStatus = await page.evaluate(async (api, tkn, id) => {
      const r = await fetch(`${api}/mesas/${id}`, { headers: { Authorization: `Bearer ${tkn}` } });
      return r.json();
    }, BACKEND_API, token, mesa.id || mesa._id);

    if (!afterStatus || afterStatus.status !== 'Reservada') throw new Error(`Status change not reflected in backend (got: ${afterStatus?.status})`);
    ...
    await Promise.all([
      page.waitForResponse(resp => resp.url().includes(`/mesas/${mesa.id || mesa._id}`) && resp.request().method() === 'DELETE' && resp.status() < 500, { timeout: 8000 }),
      delModal.locator('button:has-text("Eliminar")').click()
    ]);

    // Verify deletion via backend
    const listAfterDelete = await page.evaluate(async (api, tkn) => {
      const r = await fetch(`${api}/mesas`, { headers: { Authorization: `Bearer ${tkn}` } });
      return r.json();
    }, BACKEND_API, token);
