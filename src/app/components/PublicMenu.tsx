...
import { ShoppingCart, Search, Plus, Minus, X, User, LogOut, Loader2, ArrowRight, ChefHat, Trash2 } from 'lucide-react'
  ...
  const processCheckout = async () => {
    try {
      ...
      await api.post('/pedidos/checkout', {
        ...
        coordenadasEntrega: { lat: -17.3895, lng: -66.1568 }, // Hardcodeado hasta integrar mapa
        total: cartTotal
      })
      ...
      <header className="sticky top-0 z-40 transition-all duration-300" style={{ backgroundColor: 'rgba(234, 212, 196, 0.85)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(217, 108, 74, 0.15)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {token ? (
              <div className="flex items-center gap-3">
                <span className="text-sm font-bold text-[#4B2E2D] hidden sm:block">Hola, {user?.nombre}</span>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-end">
          <div className="bg-white w-full max-w-md h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            <div className="p-6 border-b flex items-center justify-between bg-[#FCE4D6]/30">
              ...
              <button onClick={() => setIsCartOpen(false)} className="p-2 text-gray-400 hover:text-[#4B2E2D] transition-colors bg-white rounded-full shadow-sm">
                <X size={20} />
              </button>
            </div>
