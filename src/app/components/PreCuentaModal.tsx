    ...
    if (isOpen) {
      ...
      const fetchCajeros = async () => {
        ...
        try {
          ...
          const cajerosActivos = data.filter(
            (u: any) => u.rol.toLowerCase() === 'cajero' && u.estado
          )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!activeBackendOrder) {
        toast.error(
          'No hay un pedido activo en la base de datos (Posiblemente ya fue cobrado). Cierra el panel y usa "Liberar mesa".'
        )
        setIsSubmitting(false)
        return
      }
      ...
      toast.error(
        error.response?.data?.mensaje || error.message || 'Error al enviar la cuenta al cajero.'
      )
    } finally {
      setIsSubmitting(false)
    }
      ...
      <div className="bg-white w-full max-w-[500px] rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
        <div className="bg-gradient-to-r from-[#6B3E2E] to-[#4B2E2D] p-5 text-white flex justify-between items-center relative">
          <div>
            ...
            <p className="text-white/70 text-xs font-medium mt-1">
              Completa los datos y asigna un cajero
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto max-h-[80vh]">
          ...
          <div className="space-y-4 mb-6">
            <h3 className="font-bold text-[#4B2E2D] border-b pb-2 text-sm uppercase tracking-wider">
              Datos de Facturación
            </h3>
            <div>
              <label className="block text-xs font-bold text-[#4B2E2D] mb-1.5 ml-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#D96C4A] focus:outline-none"
                />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    value={ci}
                    onChange={(e) => handleNumerosChange(e, setCi)}
              ...
              <div>
                ...
                <input
                  type="text"
                  value={nit}
                  onChange={(e) => handleNumerosChange(e, setNit)}
                  placeholder="Solo números"
                  className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#D96C4A] focus:outline-none"
                />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              <label className="flex items-center gap-1 text-[11px] font-bold text-[#4B2E2D] uppercase mb-2">
                <Percent size={14} className="text-green-600" /> Descuento
              </label>
              <div className="flex bg-gray-100 p-1 rounded-lg">
                {[0, 10, 15].map((pct) => (
                  <button
                    ...
                    className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-colors ${discountPercent === pct ? 'bg-white shadow-sm text-green-600' : 'text-gray-500'}`}
                  >
                    {pct}%
                  </button>
                ))}
            ...
            <div>
              <label className="flex items-center gap-1 text-[11px] font-bold text-[#4B2E2D] uppercase mb-2">
                <Wallet size={14} className="text-[#D96C4A]" /> Propina
              </label>
              <div className="flex bg-gray-100 p-1 rounded-lg">
                {[0, 5, 10].map((pct) => (
                  <button
                    ...
                    className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-colors ${tipPercent === pct ? 'bg-white shadow-sm text-[#D96C4A]' : 'text-gray-500'}`}
                  >
                    {pct}%
                  </button>
                ))}
          ...
          <div className="bg-[#FFF5F0] rounded-xl p-4 mb-6 border border-[#FCE4D6]">
            ...
            {tipPercent > 0 && (
              <div className="flex justify-between text-sm mb-2 text-[#D96C4A]">
                <span>Propina ({tipPercent}%)</span>{' '}
                <span className="font-bold">+Bs. {montoPropina.toFixed(2)}</span>
              </div>
            )}
          ...
          <div className="mb-6">
            <h3 className="font-bold text-[#4B2E2D] border-b pb-2 text-sm uppercase tracking-wider mb-3">
              Enviar A:
            </h3>
            {isLoadingCajeros ? (
              <div className="flex justify-center py-4">
                <Loader2 className="animate-spin text-[#D96C4A]" />
              </div>
            ) : cajeros.length === 0 ? (
              <div className="bg-red-50 p-3 rounded-xl border border-red-100 text-red-600 text-sm flex gap-2">
                <AlertCircle size={16} className="shrink-0 mt-0.5" /> No hay cajeros activos
              ...
              <div className="grid grid-cols-2 gap-2">
                {cajeros.map((c) => (
                  ...
                  >
                    <span
                      className={`font-bold text-sm ${selectedCajero === (c._id || c.id) ? 'text-[#D96C4A]' : 'text-[#4B2E2D]'}`}
                    >
                      {c.nombre}
                    </span>
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-3.5 font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={isSubmitting || cajeros.length === 0}
              className="flex-[2] flex items-center justify-center gap-2 py-3.5 font-black text-white bg-gradient-to-r from-[#D96C4A] to-[#C25838] hover:opacity-90 rounded-xl shadow-lg disabled:opacity-50"
            >
              {isSubmitting ? (
                <Loader2 className="animate-spin" size={20} />
              ) : (
                <>
                  <Send size={20} /> Enviar al Cajero
                </>
              )}
            </button>
          </div>
