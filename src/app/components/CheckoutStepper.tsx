    ...
    <div className="flex flex-col h-full flex-1 min-h-0 animate-in slide-in-from-right duration-300">
      ...
      {step > 1 && (
        <div className="p-6 bg-white border-t border-gray-100 shrink-0 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
          {step === 2 && (
            <button disabled={!deliveryInfo.address || deliveryInfo.distance === 0} onClick={() => setStep(3)} className="w-full py-4 bg-[#D96C4A] text-white rounded-xl font-black disabled:opacity-50 transition-opacity">Confirmar Ubicación</button>
          )}
          {step === 3 && (
            <button disabled={!billingInfo.name || !billingInfo.phone} onClick={() => setStep(4)} className="w-full py-4 bg-[#D96C4A] text-white rounded-xl font-black disabled:opacity-50 transition-opacity">Continuar a Pago</button>
          )}
          {step === 4 && (
            <button onClick={() => setStep(5)} className="w-full py-4 bg-[#D96C4A] text-white rounded-xl font-black transition-opacity">Revisar Pedido</button>
          )}
          {step === 5 && (
            <button onClick={processCheckout} className="w-full py-4 bg-[#D96C4A] text-white rounded-xl font-black text-lg shadow-lg hover:bg-[#b5462f] transition-all">Confirmar Pedido</button>
          )}
        </div>
      )}
    </div>
