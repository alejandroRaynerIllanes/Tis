...
export function CashierView() {
    ...
    try {
      ...
      if (selectedBill) {
        const stillPending = pending.find(
          (p: any) => p.pedidoId === selectedBill.pedidoId || p._id === selectedBill._id
        )
    ...
    const handleCuentaSolicitada = (pedidoActualizado: any) => {
      ...
      setPendingBills((prev: any[]) => {
        const existe = prev.some(
          (p) => (p.pedidoId || p._id) === (pedidoActualizado.pedidoId || pedidoActualizado._id)
        )
        if (existe)
          return prev.map((p) =>
            (p.pedidoId || p._id) === (pedidoActualizado.pedidoId || pedidoActualizado._id)
              ? pedidoActualizado
              : p
          )
  ...
  if (isRegisterClosed) {
    return (
      <div className="flex flex-col items-center justify-center h-[100dvh] bg-[#FCE4D6] p-6 text-center relative overflow-hidden">
        <div
          className="absolute inset-0 z-0 bg-cover bg-center bg-no-repeat blur-[6px] scale-[1.02] pointer-events-none opacity-40"
          style={{
            backgroundImage:
              'url(https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080)'
          }}
        />
        <div className="relative z-10 bg-white/90 backdrop-blur-md p-10 rounded-[32px] shadow-2xl border-2 border-[#D96C4A]/20 max-w-lg w-full animate-in zoom-in-95 duration-500">
          ...
          <h1 className="text-3xl font-black text-[#4B2E2D] mb-4 tracking-tight">
            Caja en reparación
          </h1>
          <p className="text-[#4B2E2D]/70 text-lg font-medium mb-8 leading-snug">
            Disculpe las molestias. Esta caja se encuentra inactiva y no puede recibir ni procesar
            operaciones.
          </p>
          <button
            onClick={handleLogout}
            className="flex items-center justify-center gap-2 w-full py-4 bg-[#4B2E2D] hover:bg-[#3A2222] text-white rounded-xl font-black transition-all shadow-lg active:scale-95 text-lg"
          >
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#FCE4D6] font-sans selection:bg-[#E57C5D] selection:text-white relative overflow-hidden">
      <div
        className="absolute inset-0 z-0 bg-cover bg-center bg-no-repeat blur-[2px] scale-[1.02] pointer-events-none opacity-20"
        style={{
          backgroundImage:
            'url(https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080)'
        }}
      ...
      <header className="px-6 py-5 bg-gradient-to-r from-[#4B2E2D] to-[#6B3E2E] text-white flex items-center justify-between z-10 shadow-lg shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={handleLogout}
            className="flex items-center gap-2 px-4 py-2 bg-red-500/20 hover:bg-red-500/40 text-red-100 rounded-xl font-bold transition-all text-sm border border-red-500/30"
          >
      ...
      <main className="flex-1 flex flex-col lg:flex-row p-4 sm:p-6 gap-6 overflow-hidden z-10">
        <section className="flex-1 flex flex-col min-w-0 gap-6 overflow-hidden">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 shrink-0">
            {[
              {
                label: 'Caja Actual (Total)',
                amount: stats.totalDia,
                icon: <TrendingUp size={20} className="text-emerald-600" />,
                bg: 'bg-emerald-50 border-emerald-200'
              },
              {
                label: 'Efectivo',
                amount: stats.efectivo,
                icon: <Banknote size={20} className="text-[#D96C4A]" />,
                bg: 'bg-white border-[#FCE4D6]'
              },
              {
                label: 'Tarjeta',
                amount: stats.tarjeta,
              ...
              <div
                key={i}
                className={`p-4 rounded-2xl border shadow-sm ${stat.bg} flex flex-col gap-2`}
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-[#4B2E2D]/60 uppercase">
                    {stat.label}
                  </span>
                  <div className="p-1.5 bg-white rounded-lg shadow-sm">{stat.icon}</div>
                </div>
                <span className="text-xl font-black text-[#4B2E2D]">
                  Bs. {stat.amount.toFixed(2)}
                </span>
              </div>
            ))}
          </div>
              ...
              {pendingBills.map((bill) => {
                ...
                const timeWaiting =
                  bill.tiempoEsperaMinutos !== undefined
                    ? `Hace ${bill.tiempoEsperaMinutos} min`
                    : new Date(bill.updatedAt || bill.createdAt).toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit'
                      })
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg ${isSelected ? 'bg-[#D96C4A] text-white' : 'bg-[#FCE4D6] text-[#4B2E2D]'}`}
                      >
                        {mesaName.replace('Mesa ', '').replace('VIP ', 'V')}
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="font-black text-[#4B2E2D] text-lg leading-none">
                            {mesaName}
                          </h3>
                        </div>
                        <p className="text-xs font-semibold text-gray-500 mt-1 flex items-center gap-1">
                          <User size={12} /> {waiterName} <span className="mx-1">•</span>{' '}
                          <Clock size={12} /> {timeWaiting}
                        </p>
                        <p
                          className="text-[11px] font-black text-[#D96C4A] mt-1 tracking-tight"
                          title="ID del Pedido"
                        >
                          {bill.codigo || `PED-${String(bId).slice(-4).toUpperCase()}`}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <p
                        className={`font-black text-xl ${isSelected ? 'text-[#D96C4A]' : 'text-[#4B2E2D]'}`}
                      >
                        Bs. {bill.total.toFixed(2)}
                      </p>
                    </div>
                  </div>
                )
          ...
          {selectedBill ? (
              ...
              <div className="bg-[#4B2E2D] p-5 text-white shrink-0">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-lg font-black text-[#E57C5D]">
                      {selectedBill.codigo ||
                        `PED-${String(selectedBill.pedidoId || selectedBill._id)
                          .slice(-4)
                          .toUpperCase()}`}
                    </p>
                  </div>
                ...
                <div className="mt-3 bg-white/10 p-3 rounded-xl">
                  <div className="flex items-center gap-2 mb-1">
                    <User size={14} className="text-[#E57C5D]" />{' '}
                    <span className="font-bold text-sm">
                      {selectedBill.clienteNombre || 'Consumidor Final'}
                    </span>
                  </div>
                  <div className="flex gap-4">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs font-medium text-white/80">
                        CI: {selectedBill.clienteCI || 'S/N'}
                      </span>
                    </div>
                    {selectedBill.clienteNIT && (
                      <div className="flex items-center gap-1.5">
                        <Receipt size={12} className="text-white/60" />{' '}
                        <span className="text-xs font-medium text-white/80">
                          NIT: {selectedBill.clienteNIT}
                        </span>
                      </div>
                    )}
                  </div>
              ...
              <div className="p-5 shrink-0 bg-white space-y-4">
                <div className="bg-[#F9F9F9] rounded-xl p-4 border border-gray-100 space-y-2">
                  ...
                  <div className="pt-2 border-t border-dashed border-gray-300 flex justify-between items-end">
                    <span className="font-black text-[#4B2E2D] uppercase tracking-wider text-xs mb-1">
                      Total a Cobrar
                    </span>
                ...
                <div>
                  <label className="block text-[11px] font-bold text-[#4B2E2D] uppercase mb-2">
                    Método de pago
                  </label>
            ...
            <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-gray-50">
              ...
              <p className="text-sm font-medium text-gray-500 max-w-[250px]">
                Selecciona una mesa de la lista de pendientes para procesar su pago y emitir la
                factura.
              </p>
            </div>
          )}
          ...
          <div className="bg-white rounded-[32px] w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="p-8 flex flex-col items-center">
              ...
              <p className="text-xs font-bold text-gray-500 text-center max-w-[280px] leading-relaxed">
                Escanee este código QR con su aplicación bancaria (o cámara del celular) para
                acceder al pago seguro simulado.
              </p>
            </div>
            ...
            <div className="p-6 bg-gray-50 border-t border-gray-100 flex flex-col gap-3">
              <button
                ...
                disabled={isProcessing}
                className="w-full py-3.5 rounded-xl bg-white border-2 border-[#D96C4A] text-[#D96C4A] hover:bg-[#FFF5F0] font-black transition-all flex items-center justify-center gap-2 disabled:opacity-50 shadow-sm"
              >
                {isProcessing ? (
                  <div className="w-5 h-5 border-2 border-[#D96C4A]/30 border-t-[#D96C4A] rounded-full animate-spin" />
                ) : (
                  <Printer size={20} />
                )}{' '}
                Imprimir QR para Mesa
              </button>
              <button
              ...
              >
                {isProcessing ? (
                  ...
                  <>
                    <CheckCircle2 size={22} /> Forzar Pago Manual
                  </>
                )}
              </button>
              <button
                onClick={() => setIsQRModalOpen(false)}
                disabled={isProcessing}
                className="w-full py-3 rounded-xl text-gray-500 font-bold hover:bg-gray-200 transition-all disabled:opacity-50"
              >
                Cerrar QR
              </button>
            </div>
            ...
            <div className="bg-[#4B2E2D] px-6 py-6 text-center relative overflow-hidden shrink-0">
              ...
              <div className="relative z-10 flex flex-col items-center">
                ...
                <p className="text-[#D96C4A] text-sm font-black mt-1 bg-[#D96C4A]/10 px-3 py-1 rounded-full border border-[#D96C4A]/20">
                  {processedBill.codigo ||
                    `PED-${String(processedBill.pedidoId || processedBill._id)
                      .slice(-4)
                      .toUpperCase()}`}
                </p>
              </div>
            ...
            <div className="p-6 bg-white relative flex-1 overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:bg-black/10">
              <div className="absolute -top-3 left-0 right-0 h-3 flex justify-around overflow-hidden">
                {Array.from({ length: 25 }).map((_, i) => (
                  <div key={i} className="w-3 h-3 bg-white rounded-full -mt-1.5 shadow-inner"></div>
                ))}
              </div>
              ...
              <div className="mb-6 space-y-2">
                <div className="flex justify-between items-center text-sm border-b border-gray-100 pb-2">
                  <span className="font-semibold text-gray-500">Cliente</span>
                  <span className="font-black text-[#4B2E2D]">
                    {processedBill.clienteNombre || 'Consumidor Final'}
                  </span>
                </div>
                <div className="flex justify-between items-center text-sm border-b border-gray-100 pb-2">
                  <span className="font-semibold text-gray-500">CI / NIT</span>
                  <span className="font-black text-[#4B2E2D]">
                    {processedBill.clienteCI || processedBill.clienteNIT || 'S/N'}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-[#FFF5F0] rounded-2xl p-4 border border-[#FCE4D6] space-y-2 mb-6">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-bold text-[#4B2E2D]">
                    Bs. {(processedBill.subtotalCierre || processedBill.total || 0).toFixed(2)}
                  </span>
                </div>
                {(processedBill.montoDescuento || 0) > 0 && (
                  <div className="flex justify-between items-center text-xs text-green-600">
                    ...
                    <span className="font-bold">
                      - Bs. {(processedBill.montoDescuento || 0).toFixed(2)}
                    </span>
                  </div>
                )}
                {(processedBill.montoPropina || 0) > 0 && (
                  <div className="flex justify-between items-center text-xs text-[#D96C4A]">
                    ...
                    <span className="font-bold">
                      + Bs. {(processedBill.montoPropina || 0).toFixed(2)}
                    </span>
                  </div>
                )}
                <div className="pt-2 border-t border-[#D96C4A]/20 flex justify-between items-end">
                  <div>
                    <span className="font-black text-[#4B2E2D] uppercase text-[10px] tracking-wider block mb-0.5">
                      Método de Pago
                    </span>
                    <span className="bg-white text-[#D96C4A] font-black text-[10px] px-2 py-0.5 rounded uppercase border border-[#D96C4A]/30">
                      {processedBill.paymentMethod || 'QR'}
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="font-black text-gray-400 uppercase text-[10px] tracking-wider block mb-0.5">
                      Total Final
                    </span>
                    <span className="font-black text-2xl text-[#D0543A] leading-none">
                      Bs.{' '}
                      {(
                        (processedBill.subtotalCierre || processedBill.total || 0) -
                        (processedBill.montoDescuento || 0) +
                        (processedBill.montoPropina || 0)
                      ).toFixed(2)}
                    </span>
                  </div>
                </div>
