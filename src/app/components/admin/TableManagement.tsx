...
export function TableManagement({ locations, setLocations }: TableManagementProps) {
  ...
  const handleSaveTable = async (e: React.FormEvent) => {
    ...
    try {
      if (tableEditingId) {
        await updateTable(tableEditingId, { name: tableFormData.number, capacity: Number(tableFormData.capacity), location: tableFormData.locationId, type: tableFormData.tableType });
      } else {
        await createTable({ name: tableFormData.number, capacity: Number(tableFormData.capacity), location: tableFormData.locationId, type: tableFormData.tableType });
      }
      setVipLimitError(false);
  ...
  return (
    <>
          ...
          <div className="bg-white w-full max-w-[400px] border-[6px] border-[#4B2E2D] rounded-3xl p-8 relative shadow-2xl">
            ...
            <form className="space-y-5" onSubmit={handleSaveTable}>
              <div>
                ...
                <input type="text" required value={tableFormData.number} onChange={(e) => setTableFormData({ ...tableFormData, number: e.target.value })} className="w-full px-4 py-3 rounded-xl border-2 border-[#E57C5D]" />
              </div>

              <div>
                <label className="block text-sm font-bold text-[#4B2E2D] mb-2">Capacidad</label>
                <input type="number" required min="1" max="20" value={tableFormData.capacity} onChange={(e) => setTableFormData({ ...tableFormData, capacity: Number(e.target.value) })} className="w-full px-4 py-3 rounded-xl border-2 border-[#E57C5D]" />
              </div>
              ...
              <div className="flex justify-end gap-4 mt-4">
                <button type="button" onClick={() => setIsTableModalOpen(false)} className="px-6 py-3 font-bold text-[#4B2E2D]/60">Cancelar</button>
                <button type="submit" className="px-8 py-3 bg-[#D0543A] text-white font-bold rounded-xl">Guardar</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {tableToDelete !== null && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"><div className="bg-[#FCE4D6] w-full max-w-[400px] border-[4px] border-[#D0543A] rounded-3xl p-8 relative flex flex-col items-center text-center"><AlertTriangle size={32} className="text-[#D0543A] mb-4"/><h2 className="text-2xl font-bold mb-3 text-[#4B2E2D]">¿Eliminar Mesa?</h2><div className="flex gap-4 w-full mt-4"><button onClick={() => setTableToDelete(null)} className="flex-1 py-3 px-4 font-bold border-2 border-[#4B2E2D] rounded-xl text-[#4B2E2D]">Cancelar</button><button onClick={async () => { try { if (tableToDelete) await deleteTable(tableToDelete); setTableToDelete(null); } catch (err) { console.error(err); alert('No se pudo eliminar la mesa.'); } }} className="flex-1 py-3 px-4 bg-[#D0543A] text-white font-bold rounded-xl">Eliminar</button></div></div></div>
      )}
